...
class cashbook:
    ...
    @staticmethod
    def find_summary(user_id, summary):
        with DBConnector(dbName='db_%s' % project.name()) as con, con.cursor() as cursor:
            cursor.execute('''
                select * from table_cashbook
                where user_id = %s and
                summary = %s
                order by `date` asc; ''',
                (user_id, summary,))
    ...
    @staticmethod
    def find_summary_datetime(user_id, summary, date):
        cb_list = []
        date = int(date)
        tmp = cashbook.find_summary(user_id, summary)
        for t in tmp:
            if t.attr['ym'] == date:
                cb_list.append(t)
    ...
    @staticmethod
    def select_flower(user_id):
        flowers = ['flower1.jpg', 'flower2.jpg', 'flower3.jpg', 'flower4.jpg']
        result = 0
        cb_list = cashbook.select_by_user_id(user_id)
        for cb in cb_list:
            result += cb.attr['amount']

        if result < 1001: return flowers[0]
        elif result < 5001: return flowers[1]
        elif result < 10001: return flowers[2]
        else: return flowers[3]